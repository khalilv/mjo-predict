...
def subset_romi_data(df, start_date, end_date, compute_statistics):
    ...
    phase = np.arctan2(subset_df['ROMI2'], subset_df['ROMI1'])
    ...
    if compute_statistics:
        mean = subset_df[['ROMI1', 'ROMI2', 'amplitude', 'year']].mean()
        std = subset_df[['ROMI1', 'ROMI2', 'amplitude', 'year']].std()
...
def main():
    ...
    input_df = load_romi_indices(input_filepath)
    ...
    # Save normalization statistics (currently not normalizing ROMI1/ROMI2/amplitude)
    ...
    noaa_df = load_romi_indices(noaa_filepath)
    aligned_df = pd.merge(input_df, noaa_df, left_index=True, right_index=True,
                          how='inner', suffixes=('_ours', '_noaa')).dropna()

    bcorr = compute_bcor(
        predict_rmm1=aligned_df['ROMI1_ours'].values,
        ground_truth_rmm1=aligned_df['ROMI1_noaa'].values,
        predict_rmm2=aligned_df['ROMI2_ours'].values,
        ground_truth_rmm2=aligned_df['ROMI2_noaa'].values
    )
    ...
    correlation_scatter_plot(
        pred_rmm1=aligned_df['ROMI1_ours'].values,
        gt_rmm1=aligned_df['ROMI1_noaa'].values,
        pred_rmm2=aligned_df['ROMI2_ours'].values,
        gt_rmm2=aligned_df['ROMI2_noaa'].values,
