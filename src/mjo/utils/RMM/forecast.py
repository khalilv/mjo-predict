    ...
    for start_date in start_dates:
        ...
        if os.path.isdir(root) and not start_date.startswith('.'):
            ...
            for member in tqdm(range(51), f'Processing ensemble members for {forecast_dir}'):
                ...
                with xr.open_mfdataset(forecast_files, combine='by_coords', parallel=True) as forecast_ds:
                    init_time = forecast_ds['time'].isel(time=0)
                    ...
                    gt_olr_data_slice = gt_olr_data.sel(time=period).load()
                    gt_u850_data_slice = gt_u850_data.sel(time=period).load()
                    gt_u200_data_slice  = gt_u200_data .sel(time=period).load()
                    ...
                    gt_olr_regridder = xe.Regridder(gt_olr_data_slice, {'lat': target_lat, 'lon': target_lon}, 'bilinear', periodic=True)
                    gt_u850_regridder = xe.Regridder(gt_u850_data_slice, {'lat': target_lat, 'lon': target_lon}, 'bilinear', periodic=True)
                    gt_u200_regridder = xe.Regridder(gt_u200_data_slice, {'lat': target_lat, 'lon': target_lon}, 'bilinear', periodic=True)
                    ...
                    gt_olr_data_2p5d = gt_olr_regridder(gt_olr_data_slice)
                    gt_u850_data_2p5d = gt_u850_regridder(gt_u850_data_slice)
                    gt_u200_data_2p5d = gt_u200_regridder(gt_u200_data_slice)
