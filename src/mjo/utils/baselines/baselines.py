...
def load_data(forecast_dir, ground_truth_path, start_date, end_date, member=None, use_grouping=False, filter_low_amplitude_samples=False, include_doy=False):
...
def run_inference(forecast_dir, save_dir, ground_truth_path, model, start_date, end_date, member='mean', method='mean_bias', use_grouping=False, include_doy=False):
...
root_save_dir = "/glade/derecho/scratch/kvirji/mjo-predict/exps/production/2019-2021/baselines"
...
train_forecast, train_target = load_data(forecast_dir, ground_truth_path, train_start, train_end, member, use_grouping=True, filter_low_amplitude_samples=filter_low_amplitude_samples)
...
train_forecast, train_target = load_data(forecast_dir, ground_truth_path, train_start, train_end, member, use_grouping=True, filter_low_amplitude_samples=filter_low_amplitude_samples, include_doy=True)
mlr_model = fit_multilinear_bias_model(train_forecast, train_target)
save_dir = f'{root_save_dir}/{n_groups}/MLR_with_doy/'
run_inference(forecast_dir, save_dir, ground_truth_path, mlr_model, train_end, test_end, member, method='mlr', use_grouping=True, include_doy=True)

train_forecast, train_target = load_data(forecast_dir, ground_truth_path, train_start, train_end, member, use_grouping=True, filter_low_amplitude_samples=filter_low_amplitude_samples, include_doy=False)
mlr_model = fit_multilinear_bias_model(train_forecast, train_target)
save_dir = f'{root_save_dir}/{n_groups}/MLR/'
run_inference(forecast_dir, save_dir, ground_truth_path, mlr_model, train_end, test_end, member, method='mlr', use_grouping=True, include_doy=False)